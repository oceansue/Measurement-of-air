...
Description: 用于2.4TFT彩屏触摸控制。
Others: 
引脚接口。主控芯片为STM32F103ZEXX
#define DCLK   PAout(4)   	//PA5 
#define CS  	 PAout(5)   	//PA4 
#define DIN 	 PAout(6)   	//PA7 
#define DOUT 	 PAin(3)    	//PA2
#define Penirq PAin(7)   		//PA3 
Function List: 
1. u8 tpstate(void);        //获取Penirq中断
2. void spistar(void);      //SPI开始
3. void Touch_Adjust(void); //触摸屏校正
4. void point(void);        //绘图函数
5. u16 ReadFromCharFrom7843(void);  //读取数据     
6. void touch_int(void);    //触摸引脚初始化
History: 
*************************************************/
...
u16 vx=15489,vy=11106;  //比例因子，此值除以1000之后表示多少个AD值代表一个像素点
u16 chx=199,chy=3871;   //默认像素点坐标为0时的AD起始值
